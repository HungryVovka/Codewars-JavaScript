// -----------------------------------------------------------
// In this lesson we learn three methods of Math: pow() sqrt() and cbrt(). Their usage is very simple: sqrt() returns the square root of a number; 
// cbrt() returns the cube root of a number; pow() returns the base to the exponent power.
// 
...
// 
// The second parameters of the pow() can be used as a fractional or decimal fraction, thus getting the same results as sqrt() and cbrt(). See 
// example:
// 
// var n=64;
...
// Due to the numerical precision of JS, the error will be produced in the calculation. This is a problem that can't be avoided. We should pay attention to 
// this problem in use, look at the following example:
// 
// function isCube(m,n){
...
// 
// This method can also be used to verify the number of squares. The following three methods are used to verify that the square root of a number is an 
// integer:
// 
// function intRoot1(n){
...
// 
// Task
// Coding in function cutCube. function accept 2 parameter: volume and n. volume is the volume of a cube. If we cut the cube into n block. please 
...
// 
// The two examples above seems to meet our requirements, but please note: a cube is unable to evenly divided into 50 pieces or 4 pieces. Only cubic 
// numbers(such as 8,27,64,125,216...) can be used to divide the cube evenly.
// -----------------------------------------------------------

function cutCube(volume, n){
  let x = Math.cbrt(volume / n);
  let y = Math.cbrt(n);
  return x % 1 == 0 && y % 1 == 0;
}